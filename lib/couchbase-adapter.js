...
module.exports = CoreObject.extend({
    ...
    upload: function(projectName, indexHTMLPath, manifestKey, revisionsKey, currentKey, revision) {
        return this._readFileContents(indexHTMLPath)
            .then(this._uploadIfNotAlreadyInManifest.bind(this, projectName, manifestKey, revisionsKey, currentKey, revision));
    },
    ...
    _ensureManifestExists: function(manifestKey, revisionsKey, currentKey) {
        var self = this;
        return RSVP.Promise.resolve(self._findManifest(manifestKey))
            .catch(function() {
    ...
    _uploadIfNotAlreadyInManifest: function(projectName, manifestKey, revisionsKey, currentKey, revision, indexHTML) {
        var self = this;
        return self._ensureManifestExists(manifestKey, revisionsKey, currentKey)
            .then(function(manifestDoc) {
                if (!self._revisionExists(manifestDoc, revisionsKey, revision)) {
                    return self._uploadRevision(projectName, revision, indexHTML, manifestDoc, manifestKey, revisionsKey, currentKey)
                        .finally(function() {
     ...
     * @param projectName {String} the name of the project being deployed. This
     *                             is used as a prefix for the key for the doc
     *                             containing index.html contents.
     ...
     */
    _uploadRevision: function(projectName, revision, indexHTML, manifestDoc,
                       manifestKey, revisionsKey, currentKey) {
        var self = this,
            key = projectName + ":" + revision;

        return new RSVP.Promise(function(resolve, reject) {
            self.client.insert(key, { content: indexHTML.toString() }, function(err, res) {
                if (err) {
                    reject(err.message);
                } else {
                    resolve();
